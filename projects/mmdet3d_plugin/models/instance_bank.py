...
class InstanceBank(nn.Module):
    ...
    def get(self, batch_size, metas=None, dn_metas=None):
        ...
        if (
            ...
            if (
                ...
                num_dn_group, num_dn = dn_metas["dn_anchor"].shape[1:3] #时序的噪声群组是在5个中保留了3个
                dn_anchor = self.anchor_handler.anchor_projection(
                    dn_metas["dn_anchor"].flatten(1, 2),
                    [T_temp2cur],
                    time_intervals=[-time_interval],
                )[0] #同样对时序传播的噪声实例进行运动补偿
    ...
    def cache(
        ...
        (
            self.confidence,
            (self.cached_feature, self.cached_anchor, self.cached_anchor_embedding),
        ) = topk(confidence, self.num_temp_instances, instance_feature, anchor, anchor_embedding)
