...
class SparseBox3DDecoder(object):
    ...
    def decode(
        ...
        for i in range(bs):
            ...
            if xt is not None and len(kt_preds):
                mask = indices[i] < 600
                kt_preds = kt_preds[i][0][indices[i][mask] // num_cls]
                xt = xt[i, indices[i][mask] // num_cls][..., [0, 1, 8, 9]]
                zt = box[:, [0, 1, 8, 9]][mask]
                h_xt = xt
                update_box = xt + torch.matmul(
                    kt_preds, (zt - h_xt)[..., None]
                ).squeeze(-1)
                box[mask, :2] = update_box[:, :2]
                box[mask, 8:10] = update_box[:, 2:]
