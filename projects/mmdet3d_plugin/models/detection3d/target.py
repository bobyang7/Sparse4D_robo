...
class SparseBox3DTarget(BaseTargetWithDenoising):
    ...
    def get_dn_anchors(self, cls_target, box_target, gt_instance_id=None):
        ...
        if self.num_dn_groups > 1:
            cls_target = cls_target.tile(self.num_dn_groups, 1) #tile([1,2,3]) = [1,2,3,1,2,3]不是[1,1,2,2,3,3]
        ...
        if self.add_neg_dn:
            ...
            dn_anchor = torch.cat([dn_anchor, box_target + noise_neg], dim=1)# 拼接正负噪声，各32个
            num_gt *= 2
        ...
        dn_cls_target = -torch.ones_like(cls_target) * 3 #[40,32] 上面的回归考虑负噪声，这里的分类只考虑正噪声
        if gt_instance_id is not None:
            dn_id_target = -torch.ones_like(gt_instance_id) #[40,32] 这里也只考虑正噪声
        ...
        for i in range(dn_anchor.shape[0]):# 给40个cost矩阵依次匈牙利，并给其中的64个噪声分配gt，只能分配到32个，理想情况下是正噪声这32个
        ...
        if gt_instance_id is not None:
            ...
            dn_id_target = None
        valid_mask = dn_cls_target >= 0 # dn_cls_target:(正例噪声的类别，填补到32的正例噪声的类别[-1]，负例噪声的类别[-3]，填补到32的负例噪声的类别[-3]) * 5
        if self.add_neg_dn:
            cls_target = (
        ...
        attn_mask = dn_box_target.new_ones( #参考DN-DETR笔记，不同组的query不能看到彼此
    ...
    def update_dn(
        ...
        match = temp_dn_id[..., None] == dn_id[:, :num_temp_dn_groups, None] #temp_dn_id是[8,3,64],dn_id是[8,5,64],因此在判断[8,3,64,1]==[8,3,1,64]
        ...
        ).sum(dim=3) #[8,3,64,64,+1] * [8,3,+1,64,10],这里是点乘，不是叉乘。
        ...
        for i, (temp_meta, meta) in enumerate(zip(temp_dn_metas, dn_metas)):
            ...
            meta = torch.cat([temp_meta, meta[:, num_temp_dn_groups:]], dim=1) #将3组缓存的噪声query和2/5组实例噪声query拼接
