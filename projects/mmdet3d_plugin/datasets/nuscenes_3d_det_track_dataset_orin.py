...
class NuScenes3DDetTrackDataset(Dataset):
    ...
    def load_annotations(self, ann_file):
        data = mmcv.load(ann_file, file_format="pkl")
        data_infos = list(sorted(data["infos"], key=lambda e: e["timestamp"]))
        have_sweep = [i for i in data_infos if len(i["sweeps"]) != 0]
        sweeps = []
        for i in range(len(have_sweep)):
            sweeps += [*have_sweep[i]["sweeps"]]
    ...
    def format_results(self, results, jsonfile_prefix=None, tracking=False):
        ...
        if not ("pts_bbox" in results[0] or "img_bbox" in results[0]):
            ...
            for name in results[0]:
                ...
                # result_files.update(
                #     {
                #         name: "/tmp/tmpsgfjjin5/results/img_bbox/results_nusc.json"
                #     }
                # )
    ...
    def show(self, results, save_dir=None, show=False, pipeline=None):
        ...
        for i, result in enumerate(results):
            if not (
                (i < 580 and i > 555)
                or (i > 1030 and i < 1080)
                or (i > 1110 and i < 1135)
                or (i > 1229 and i < 1285)
            ):
                continue
